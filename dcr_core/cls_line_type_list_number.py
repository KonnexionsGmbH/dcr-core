...
class LineTypeListNumber:
    ...
    def _finish_list(self) -> None:
        ...
        for [page_idx, para_no, line_lines_idx_from, line_lines_idx_till, _] in self._entries:
            line_lines: cls_nlp_core.NLPCore.ParserLineLines = self._parser_line_pages_json[page_idx][
            ...
            for idx in range(int(line_lines_idx_from), int(line_lines_idx_till) + 1):
                line_lines[idx][cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] = cls_nlp_core.NLPCore.LINE_TYPE_LIST_NUMBER

                if core_glob.setup.is_create_extra_file_list_number:
                    text.append(line_lines[idx][cls_nlp_core.NLPCore.JSON_NAME_TEXT])
            ...
            if core_glob.setup.is_create_extra_file_list_number:
                ...
                entries.append(
                    {
                        cls_nlp_core.NLPCore.JSON_NAME_ENTRY_NO: len(entries) + 1,
                        cls_nlp_core.NLPCore.JSON_NAME_LINE_NO_PAGE_FROM: int(line_lines_idx_from) + 1,
                        cls_nlp_core.NLPCore.JSON_NAME_LINE_NO_PAGE_TILL: int(line_lines_idx_till) + 1,
                        cls_nlp_core.NLPCore.JSON_NAME_PAGE_NO: int(page_idx) + 1,
                        cls_nlp_core.NLPCore.JSON_NAME_PARA_NO: para_no,
                        cls_nlp_core.NLPCore.JSON_NAME_TEXT: " ".join(text),
                    }
                )

            self._parser_line_pages_json[page_idx][cls_nlp_core.NLPCore.JSON_NAME_LINES] = line_lines
        ...
        if core_glob.setup.is_create_extra_file_list_number:
            ...
            # }
            entry = {
                cls_nlp_core.NLPCore.JSON_NAME_NUMBER: self._rule[0].rstrip(),
                cls_nlp_core.NLPCore.JSON_NAME_LIST_NO: self.no_lists,
                cls_nlp_core.NLPCore.JSON_NAME_NO_ENTRIES: len(entries),
                cls_nlp_core.NLPCore.JSON_NAME_PAGE_NO_FROM: int(self._entries[0][0]) + 1,
                cls_nlp_core.NLPCore.JSON_NAME_PAGE_NO_TILL: int(self._entries[-1][0]) + 1,
            }

            if core_glob.setup.is_lt_list_number_file_incl_regexp:
                entry[cls_nlp_core.NLPCore.JSON_NAME_REGEXP] = self._rule[-1]

            entry[cls_nlp_core.NLPCore.JSON_NAME_ENTRIES] = entries
    ...
    def _init_anti_patterns(self) -> list[tuple[str, re.Pattern[str]]]:
        ...
        if core_glob.setup.lt_list_number_rule_file and core_glob.setup.lt_list_number_rule_file.lower() != "none":
            ...
            core_utils.terminate_fatal(
                ...
                f"file name '{core_glob.setup.lt_list_number_rule_file}'"
            )

        anti_patterns = []

        for name, regexp in cls_nlp_core.NLPCore.get_lt_anti_patterns_default_list_number(
    ...
    def _init_rules(self) -> list[LineTypeListNumber.RuleExtern]:
        ...
        return cls_nlp_core.NLPCore.get_lt_rules_default_list_number()
    ...
    def _load_anti_patterns_from_json(
        ...
        with open(lt_list_number_rule_file, "r", encoding=core_glob.FILE_ENCODING_DEFAULT) as file_handle:
            ...
            for rule in json_data[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE_ANTI_PATTERNS]:
                anti_patterns.append(
                    (
                        rule[cls_nlp_core.NLPCore.JSON_NAME_NAME],
                        re.compile(rule[cls_nlp_core.NLPCore.JSON_NAME_REGEXP]),
                    )
                )
        ...
        core_utils.progress_msg(
            ...
            + f"from the file {core_glob.setup.lt_list_number_rule_file}",
        )
        ...
        with open(lt_list_number_rule_file, "r", encoding=core_glob.FILE_ENCODING_DEFAULT) as file_handle:
            ...
            for rule in json_data[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE_RULES]:
                rules.append(
                    (
                        rule[cls_nlp_core.NLPCore.JSON_NAME_NAME],
                        rule[cls_nlp_core.NLPCore.JSON_NAME_REGEXP],
                        getattr(
                            cls_nlp_core.NLPCore,
                            "is_asc_" + rule[cls_nlp_core.NLPCore.JSON_NAME_FUNCTION_IS_ASC],
                        ),
                        rule[cls_nlp_core.NLPCore.JSON_NAME_START_VALUES],
                    )
                )

        core_utils.progress_msg(
            core_glob.setup.is_verbose_lt_list_number,
            "The list_number rules were successfully loaded from the "
            + f"file {core_glob.setup.lt_list_number_rule_file}",
        )
    ...
    def _process_line(self, line_line: dict[str, float | int | str]) -> None:  # noqa: C901
        ...
        """
        text = str(line_line[cls_nlp_core.NLPCore.JSON_NAME_TEXT])
        ...
        para_no = int(line_line[cls_nlp_core.NLPCore.JSON_NAME_PARA_NO])
    ...
    def process_document(
        ...
        if core_glob.setup.is_create_extra_file_list_number and self._lists:
            full_name = core_utils.get_full_name(
                directory_name,
                core_utils.get_stem_name(str(file_name_curr)) + "_list_number." + core_glob.FILE_TYPE_JSON,
            )
                ...
                json.dump(
                    {
                        ...
                        cls_nlp_core.NLPCore.JSON_NAME_NO_LISTS_NUMBER_IN_DOC: self.no_lists,
                        cls_nlp_core.NLPCore.JSON_NAME_LISTS_NUMBER: self._lists,
                    },
                    file_handle,
