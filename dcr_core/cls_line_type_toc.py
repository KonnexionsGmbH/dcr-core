...
class LineTypeToc:
    ...
    def __init__(
        ...
        core_glob.setup.is_toc_existing = False
    ...
    def _check_toc_candidate(self) -> None:
        ...
        for [page_no_toc, _, _, _] in self._toc_candidates:
            ...
            if page_no_toc == -1:
                if row_no != 1:
                    ...
                    core_utils.progress_msg(
                        core_glob.setup.is_verbose_lt_toc,
                        "LineTypeToc: End   check TOC candidate (!=)       " + f"={core_glob.setup.is_toc_existing}: {page_no_toc}",
                    )
                    return
        ...
        core_glob.setup.is_toc_existing = True

        core_utils.progress_msg(
            core_glob.setup.is_verbose_lt_toc,
            f"LineTypeToc: End   check TOC candidate            ={core_glob.setup.is_toc_existing}",
        )
    ...
    def _process_page_lines(self) -> None:
        """Process the page-related data - line version."""
        if core_glob.setup.is_toc_existing or self._page_no >= core_glob.setup.lt_toc_last_page:
            return
            ...
            if line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] == cls_nlp_core.NLPCore.LINE_TYPE_BODY:
                if (text := line_line[cls_nlp_core.NLPCore.JSON_NAME_TEXT]) != "":
                    ...
                    try:
                        ...
                        if core_glob.setup.is_toc_existing:
                            break
    ...
    def _process_page_table(self) -> None:
        ...
        for line_line in self._parser_line_lines_json:
            if line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] == cls_nlp_core.NLPCore.LINE_TYPE_BODY:
                if cls_nlp_core.NLPCore.JSON_NAME_ROW_NO in line_line:
    ...
    def _process_toc_candidate_line_line(self, line_line: cls_nlp_core.NLPCore.ParserLineLine, page_no_toc: int) -> None:
        ...
        Args:
            line_line (cls_nlp_core.NLPCore.LineLine):
                    Document line.
            page_no_toc (int):
                    Page number in the table of contents.
        """
        line_no = line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_NO]

        para_no = line_line[cls_nlp_core.NLPCore.JSON_NAME_PARA_NO]
    ...
    def _process_toc_candidate_table_line(self, line_line: cls_nlp_core.NLPCore.ParserLineLine) -> None:
        ...
        Args:
            line_line (cls_nlp_core.NLPCore.LineLine):
                    Document line.
        """
        row_no = line_line[cls_nlp_core.NLPCore.JSON_NAME_ROW_NO]
        ...
        try:
            self._toc_candidates[-1][0] = int(line_line[cls_nlp_core.NLPCore.JSON_NAME_TEXT])
    ...
    def _store_results(self) -> None:  # noqa: C901
        ...
        for page in self.parser_line_pages_json:
            ...
            for line_line in page[cls_nlp_core.NLPCore.JSON_NAME_LINES]:
                ...
                if self._strategy == cls_nlp_core.NLPCore.SEARCH_STRATEGY_LINES:
                    for [
                        _,
                        cand_page_no,
                        cand_para_no,
                        cand_line_no,
                    ] in self._toc_candidates:
                        if (
                            page_no == cand_page_no
                            and para_no == cand_para_no
                            and line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_NO] == cand_line_no
                        ):
                            line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] = cls_nlp_core.NLPCore.LINE_TYPE_TOC
                elif self._strategy == cls_nlp_core.NLPCore.SEARCH_STRATEGY_TABLE:
                    if cls_nlp_core.NLPCore.JSON_NAME_ROW_NO in line_line:
                        if line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] == cls_nlp_core.NLPCore.LINE_TYPE_BODY:
                            line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] = cls_nlp_core.NLPCore.LINE_TYPE_TOC
    ...
    def process_document(
        ...
        # -------------------------------------------------------------------------
        self._strategy = cls_nlp_core.NLPCore.SEARCH_STRATEGY_TABLE
        ...
        if not core_glob.setup.is_toc_existing:
            self._check_toc_candidate()
        ...
        # -------------------------------------------------------------------------
        if not core_glob.setup.is_toc_existing:
            self._strategy = cls_nlp_core.NLPCore.SEARCH_STRATEGY_LINES
            self._page_no = 0
            self._init_toc_candidate()
            for page_json in self.parser_line_pages_json:
                self._parser_line_lines_json = page_json[cls_nlp_core.NLPCore.JSON_NAME_LINES]
