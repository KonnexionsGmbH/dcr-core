...
class LineTypeHeading:
    ...
    # -----------------------------------------------------------------------------
    @staticmethod
    def _check_valid_start_value(  # noqa: C901
    ...
    def _create_toc_entry(self, level: int, text: str) -> None:
        ...
        toc_entry = {
            cls_nlp_core.NLPCore.JSON_NAME_HEADING_LEVEL: level,
            cls_nlp_core.NLPCore.JSON_NAME_HEADING_TEXT: text,
            cls_nlp_core.NLPCore.JSON_NAME_PAGE_NO: self._page_idx + 1,
        }

        if core_glob.setup.lt_heading_file_incl_no_ctx > 0:
            page_idx = self._page_idx
            line_lines: cls_nlp_core.NLPCore.ParserLineLines = core_glob.text_parser.parse_result_line_lines
        ...
        if core_glob.setup.is_lt_heading_file_incl_regexp:
            toc_entry[cls_nlp_core.NLPCore.JSON_NAME_REGEXP] = self._rules_hierarchy[level - 1][8]
    ...
    def _get_next_body_line(
        ...
        for idx in range(line_lines_idx + 1, len(line_lines)):
            line_line: cls_nlp_core.NLPCore.ParserLineLine = line_lines[idx]
        ...
        if (page_idx + 1) < self._max_page:
            ...
            line_lines_local: cls_nlp_core.NLPCore.ParserLineLines = core_glob.text_parser.parse_result_line_pages[
                page_idx_local
    ...
    def _init_rules(
        ...
        if core_glob.setup.lt_heading_rule_file and core_glob.setup.lt_heading_rule_file.lower() != "none":
            ...
            core_utils.terminate_fatal(
                f"File with heading rules is missing - " f"file name '{core_glob.setup.lt_heading_rule_file}'"
            )

        return cls_nlp_core.NLPCore.get_lt_rules_default_heading()
    ...
    def _load_anti_patterns_from_json(
        ...
        core_utils.progress_msg(
            core_glob.setup.is_verbose_lt_heading,
            "The heading anti-patterns were successfully loaded from the "
            + f"file {core_glob.setup.lt_heading_rule_file}",
        )
        ...
        with open(lt_heading_rule_file, "r", encoding=core_glob.FILE_ENCODING_DEFAULT) as file_handle:
            ...
            for rule in json_data[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE_RULES]:
                rules.append(
                    (
                        rule[cls_nlp_core.NLPCore.JSON_NAME_NAME],
                        rule[cls_nlp_core.NLPCore.JSON_NAME_IS_FIRST_TOKEN],
    ...
    def _process_line(self, line_line: dict[str, str], text: str, first_token: str) -> int:  # noqa: C901
        ...
        coord_llx_curr = line_line[cls_nlp_core.NLPCore.JSON_NAME_COORD_LLX]
    ...
    def _process_page(self) -> None:
        ...
        for line_lines_idx, line_line in enumerate(core_glob.text_parser.parse_result_line_lines):
            self._line_lines_idx = line_lines_idx
            if line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] != cls_nlp_core.NLPCore.LINE_TYPE_BODY:
                continue

            if (text := line_line[cls_nlp_core.NLPCore.JSON_NAME_TEXT]) == "":
                # not testable
                continue
            ...
            if (level := self._process_line(line_line, text, first_token)) > 0:
                line_line[cls_nlp_core.NLPCore.JSON_NAME_LINE_TYPE] = (
                    cls_nlp_core.NLPCore.LINE_TYPE_HEADER + "_" + str(level)
                )
                core_glob.text_parser.parse_result_line_lines[self._line_lines_idx] = line_line
    ...
    def process_document(
        ...
        if core_glob.setup.is_create_extra_file_heading and self._toc:
            full_name = core_utils.get_full_name(
                directory_name,
                core_utils.get_stem_name(str(file_name_curr)) + "_heading." + core_glob.FILE_TYPE_JSON,
            )
                ...
                json.dump(
                    {
                        ...
                        cls_nlp_core.NLPCore.JSON_NAME_TOC: self._toc,
                    },
                    file_handle,
